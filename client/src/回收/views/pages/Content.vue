...
export default {
      ...
      indexAjax.getArticleById(id).then((req) => {
        ...
        if (req.data.code == 0) {
          ...
          indexAjax
            ...
            .then((req) => {
              if (req.data.code == 0) {
                this.praise = req.data.data.count;
              }
            })
            .catch((req) => {
              console.log(req);
  ...
  mounted() {
    // this.sendAjax();
    axios.get(`api/article/27`).then((req) => {
      console.log(req);
    });
  },
  computed: {
  ...
  watch: {
    ...
    //   handler() {
    //     console.log("$store", this.$store.state.articleId);
    //     this.sendAjax(this.$store.state.articleId);
    //   },
    ...
    //     axios.get(`api/article/${id}`).then(req=>{
    //       console.log(req);
    //     })
