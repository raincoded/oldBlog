// const Tags = require("./../../../mysql/models/blog/Tags");
...
const { Tags, Article, Users, db } = require("./../models/models");// 数据库模型
const { ArticleService } = require('./init')
 ...
 */
exports.getTagsByPage = async function ({ params, isAdmin }) {
    let { page = 1, limit = 10, tag, articleId, distinct } = params;
    const where = {};
    ...
    const result = await Tags.findAndCountAll({
        ...
        attributes: { exclude: isAdmin ? [] : ['updatedAt', 'deletedAt'] },
        where: {
            ...where
        },
        offset: (page - 1) * limit,
 ...
 * @param {*} tag 
 */
exports.addTags = async function ({ params, isAdmin }) {
    // , {attributes: ["id", "tag", "articleId"],}
    const newObj = pick(params, 'articleId', 'tag');// 过滤需要的值
 ...
 * @param {*} id 
 */
exports.getTagsById = async function ({ id, isAdmin }) {
    id = +id;
    const testResult = validate({ id }, {
        id: {
            presence: true,
