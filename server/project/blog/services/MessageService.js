...
const { Message } = require("./../models/models");// 数据库模型
const { UserService } = require('./init')
...
// 发布留言
exports.pushMessage = async function ({ params, isAdmin }) {
    let newObj = pick(params, 'name', 'email', 'content');
...
// replyMessage 回复留言
exports.replyMessage = async function ({ params, isAdmin }) {
    let { id, content } = pick(params, 'id', 'content');
    id = +id;
...
// 根据id查询留言
exports.findMessageById = async function ({ id, isAdmin }) {
    id = +id;
    ...
    const result = await Message.findByPk(id, {
        attributes: { exclude: isAdmin ? [] : ['updatedAt', 'deleteAt'] }
    });
    return JSON.parse(JSON.stringify(result))
}
 ...
 */
exports.getMessageByPage = async function ({ params, isAdmin }) {
    let { page = 1, limit = 10, orderProp, order } = params;
    testNumber('page', page) && (page = +page);
    testNumber('limit', limit) && (limit = +limit);
    const select = {
        attributes: { exclude: isAdmin ? [] : ['deletedAt'] },
        offset: (page - 1) * limit,
        limit: +limit,
    }
